        ...
        if (vrInput) {
            if (vrInput.getPose) {
                ...
                if (pose.orientation !== null) {
                    //quaternion  四元数
                    //把设备的方向复制给camera
                    // var beta = pose.orientation[0] ? THREE.Math.degToRad(pose.orientation[0]) : 0; // X'
                    // var gamma = pose.orientation[1] ? THREE.Math.degToRad(pose.orientation[1]) : 0; // Y''
                    // var alpha = pose.orientation[2] ? THREE.Math.degToRad(pose.orientation[2]) : 0; // Z
                    // var orient = pose.orientation[3] ? THREE.Math.degToRad(pose.orientation[3]) : 0; // O
                    // var beta = object.quaternion.x + (pose.orientation[0] - object.quaternion.x) / 30;
                    // var gamma = object.quaternion.y + (pose.orientation[1] - object.quaternion.y) / 30;
                    // var alpha = object.quaternion.z + (pose.orientation[2] - object.quaternion.z) / 30;
                    // var orient = pose.orientation[3] * 1;
                    // console.log(pose.orientation);
                    // console.log([beta, gamma, alpha, orient]);

                    // object.quaternion.fromArray([beta, gamma, alpha, orient]);
